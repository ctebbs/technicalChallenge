...
public class Pee : StateMachineBehaviour {
	private Transform targetTransform;
	public float targetDistanceTolerance = 3.0f;
	GameObject dog;
	GameObject target;
	private float targetTimePee;
	...
	override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
	{
		...
		rotationSpeed = 2.0f;
		targetTimePee = 3.0f;
		targetTransform = GameObject.FindGameObjectWithTag("tree").transform; 
	...
	override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
	{
		...
		targetTimePee -= Time.deltaTime; 

		if (targetTimePee <= 0.0f) { 
			targetTimePee = 10f; 
